...
export default function StudentRegisterPage() {
    ...
    try {
      ...
      const { isSignUpComplete, userId, nextStep } = await signUp({
        ...
        options: {
          userAttributes: {
            email: email,
            birthdate: birthdate, // פורמט: YYYY-MM-DD
            "custom:role": role,
          },
        },
      });

      console.log("הרשמה הצליחה:", userId);
      setSuccess(true);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h2 style={styles.title}>הרשמה כתלמיד</h2>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <button type="submit" disabled={loading || success} style={styles.button}>{loading ? "מבצע הרשמה..." : "הרשם כתלמיד"}</button>
        </form>
